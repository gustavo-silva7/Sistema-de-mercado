...
def listar_produtos():
    """Retorna os produtos cadastrados ordenados por nome e marca."""
    with get_connection() as con:
        cursor = con.cursor()
        cursor.execute("""
            SELECT id, codigo, produto, marca, preco, quant, tipo
            FROM produtos
            ORDER BY produto ASC, marca ASC
        """)
...
def buscar_produto_por_codigo(codigo):
    """Busca um produto pelo código e retorna um dicionário ou None."""
    codigo = (codigo or "").strip()
    if not codigo:
        raise ValidacaoError("Código do produto é obrigatório.")
